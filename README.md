...
On exit (`Ctrl+C`), saves the last fetched data and fetch time to `~/.ccusage_cache.json`. On next start, that data is shown immediately and the next API call is delayed so the 2-minute interval is respected across restarts.
